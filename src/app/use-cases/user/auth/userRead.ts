...
export const getVenuesByDateService = async (date: string, venueDbRepository: ReturnType<venueDbInterface>) => {
    try {
        const venues = await venueDbRepository.getVenuesByDate(date);
        return venues;
    } catch (error) {
        throw error;
    }
